...
$20^\circ$, (2) reveals a plausible location for the progenitor, (3) detects
high-contrast gaps along the stream, and (4) indicates
the existence of stream members perturbed off the main stream track.
...
\changes{In detail, we project the solar velocity vector onto the tangent space at each star's sky position, convert to angular motion using the assumed distance, and subtract the solar component from each star's proper motion (this is done using the \package{astropy} and \package{gala} packages).}
...
We have verified that these do not map to features in dust extinction nor are
they caused by completeness variations in the \gaia\ scanning pattern:
the visible portions of the GD-1 stream are located at high Galactic latitudes
($b > 20^\circ$), and 99\% of our sample has at least 8 \gaia\ visibility
periods, promising a robust astrometric solution.
...
We use an ensemble Markov Chain Monte Carlo (MCMC) sampler with 64 walkers to generate posterior samples (\citealt{Foreman-Mackey:2013}) and run the sampler for a total of 4096 steps, discarding the first 2048 samples as ``burn-in.''
\figurename~\ref{fig:density-model} summarizes the results of this inference: top row shows histograms of the $\phi_2$ positions of stars in each $\phi_1$ range (left to right) and bottom row shows marginal posterior probability distributions for $\alpha_{\textrm{bg}}$, $\alpha_{\textrm{s}}$, and $\alpha_{\textrm{f}}$ estimated from the posterior samples for each $phi_1$ range.
TODO: shows what
}
