...
the Milky Way, largely thanks to large-area, multi-band photometric surveys
(especially the Sloan Digital Sky Survey, SDSS; \citealt{York:2000}).
...
Its long length --- and therefore its implied old dynamical age --- and its
particular orbit also make it a prime stream to search for interactions with
...
interactions with giant molecular clouds (\citealt{TODO}) or resonant
interactions with the bar (\citealt{Pearson:2017}).
It is particularly exciting, therefore, that deep photometry of the stream has
revealed interesting small-scale ``wiggles'' and density variations along a
$\approx 45^\citc$ span of the stream (\citealt{DeBoer:2017}).
, but color-magnitude only, bin stars]

In this \article, we present a the cleanest view of the GD-1 stream using
