...
- properties of GD-1 along the track, and beyond

\subsection{Global properties}
\label{sec:res_global}

- for study of global properties, we first find stream ridge points as a median $\phi_2$ every $x^\circ$ along $\phi_1$
- then we fit a second order polynomial to these ridge points, and define a stream track as the region $0.6^\circ$ above and beyond the best-fit polynomial (shown with two blue lines in Figure~\ref{fig:track-and-model})
...
\figurename~\ref{fig:track-and-model} (top panel) shows the $V$-band extinction in the
...
0.04~\textrm{mag}$, and the dispersion in $A_V$ in this region is $\sigma_{A_V} \approx
0.03~\textrm{mag}$.
...
- also show median properties along the stream 5deg window, 1deg spacings
- coordinates and proper motion vary
- these observations are reproduced with a simple model
