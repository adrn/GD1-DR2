...
We compare color-magnitude diagrams and proper motions of the spur and blob fields to the stream and background fields.
...
Stars selected from the stream fields show a clear overdensity in the CMD selection box and cluster in proper motions.
The two stream fields have a clear relative offset in proper motions, as expected from the velocity gradients shown in Figure~\ref{fig:track-and-model}.
...
GD-1 seen by \gaia\ is no longer a simple, one-dimensional structure.
Not only are the density variations, tentatively deduced in previous studies, confirmed as highly significant, but for the first time we have mapped tidal debris from a globular cluster beyond a thin stellar track: parts of GD-1 are at a distance of up to $\sim1^\circ$ from the main track.
Separation of a part of a stellar stream from the main track could be caused by the different exit velocity from the cluster, but explaining the amount of separation and velocity offset in GD-1 would require the progenitor to rotate at XX km\,s$^{-1}$, much faster than measured for the known clusters (cit).
Features off from the cold part of the stream are likely too small to be caused by chaotic dispersal from either triaxial matter distribution (cit apw) or the time-dependent influence of the rotating bar (cit pearson).
However, encounters with a massive perturber are expected to open gaps in streams (cits), so we might be seeing the expelled material in the process of phase mixing away from the stream track.
The encounter origin would be especially exciting -- since GD-1 resides in the halo, the only objects massive enough likely to cross its path are the long-searched-after dark-matter subhalos.

Finding the origin of gaps and off-track features in GD-1 will require progress on two fronts: theoretically, we need to understand the co-evolution of the complex stream system under different origin scenarios, while observing radial velocities of candidate GD-1 members would facilitate data interpretation by remove the remaining contamination and enabling studies in the space of conserved quantities.
